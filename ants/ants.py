...
# Person A: Alok Rai - cs61a-uq
# Person B: Namkyu Chang - cs61a-ur
# TA: Phillip Carpenter
# Grader: Amy Zhang
# Hi Amy, as always, thanks!
...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        """
        if insect.is_ant(): #This insect is an ant
            if self.ant is None: #No ants here
                self.ant = insect
            elif insect.can_contain(self.ant) == True: #The newcomer can contain the original ant
                insect.contain_ant(self.ant)
                self.ant = insect
            elif self.ant.can_contain(insect): #The ant here can contain
...
class Bee(Insect):
    ...
    name = 'Bee'
    watersafe = True
    duration = 0 #Special attribute for Q9
    ...
    def new_action(self, new): #A special case for Q9 when the original action needs and override
        self.action = new.__get__(self)
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        check_place = self.place # Whatever place is being checked for bees at the time
        count = 0
        while count < self.min_range:
            check_place = check_place.entrance
            count += 1
        while (count >= self.min_range) and (count <= self.max_range):
            if check_place.entrance.name == 'Hive' :
                return None
            elif len(check_place.bees) != 0:
...
class FireAnt(Ant):
   ...
   def reduce_armor(self, amount):
       if self.armor - amount <= 0:
           i = len(self.place.bees) - 1
           while i >= 0:
               self.place.bees[i].reduce_armor(self.fire_damage)
               i -= 1
...
class NinjaAnt(Ant):
   ...
   def action(self, colony):
       i = len(self.place.bees) - 1
       while i >= 0:
           self.place.bees[i].reduce_armor(1)
           i -= 1
...
def make_stun(bee):
    """Retnurn a new action method that does nothing.
